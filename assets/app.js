...
$(() => {
  ...
  function refreshCart(cart) {
    ...
    jQuery('.gift-bar span').css('width',giftcnt+'%');
     ...
     setTimeout(function(){
      if( jQuery('ul.cart_items.js-cart_items').eq(0).find('li.mini-cart__item').length>2 ){
        jQuery('.mix-match-products').hide();
      }else{
        jQuery('.mix-match-products').show();
      }
     },100);
    ...
    if (cart.item_count === 0) {
      ...
      $.each(cart.items, (index, item) => {
        ...
        + `<a href="${item.url}" class="mini-cart__item-a"><span class="initials ${item.properties['Letter Color']}">${item.properties['Letters']}</span>`;
        if (item.image) {
      ...
      $.ajax({
          ...
          success(data) {
              ...
              if( value.product_type != curr_pro && value.product_type=='Phone Cases' ){
                $.each( value.variants, function( key2, value2 ) {
                  if( value2.title == curr_pro_title && ph_pro==0 ){
                    ...
                    match_html += '</div></a>';
                    match_html += '<div class="mix-pro-price" ><div><h4>'+value.product_type+'</h4><p>You deserve a phone case made just for you.</p></div>';
                    match_html += '<h5>'+window.Shopify.formatMoney(value2.price, $('body').data('money-format')).replace('.00','').replace(',00','')+'</h5>';
                    ...
                    match_html += '<input min="1" type="hidden" id="quantity" name="quantity" value="1"><input type="submit" value="+ ADD" class="btn add_to_cart ajax-submit" /></form>';
              ...
              if( value.product_type != curr_pro && value.product_type=='Card Wallets' ){
                $.each( value.variants, function( key2, value2 ) {
                  if( value2.title == curr_pro_title && caw_pro==0 ){
                    ...
                    match_html += '<span class="silver-logo active" style="background-image:url(https://cdn.shopify.com/s/files/1/0595/8192/3483/t/2/assets/tccwearlogo-silver.png)"></span></div>';
                    match_html += '</div></a>';
                    match_html += '<div class="mix-pro-price" ><div><h4>'+value.product_type+'</h4><p>Keep your cards, coins and bills organized at all times with our sleek designed card wallets.</p></div>';
                    match_html += '<h5>'+window.Shopify.formatMoney(value2.price, $('body').data('money-format')).replace('.00','').replace(',00','')+'</h5>';
                    match_html += '<form method="post" action="/cart/add">';
                    match_html += '<input type="hidden" name="properties[Letters]" value="'+curr_pro_intial+'">';
                    match_html += '<input type="hidden" name="properties[Letter Color]" value="'+curr_pro_color+'">';
              ...
              if( value.product_type != curr_pro && value.product_type=='Card Holders' ){
                $.each( value.variants, function( key2, value2 ) {
                  if( value2.title == curr_pro_title && cah_pro==0 ){
                    ...
                    match_html += '<span class="silver-logo active" style="background-image:url(https://cdn.shopify.com/s/files/1/0595/8192/3483/t/2/assets/tccwearlogo-silver.png)"></span></div>';
                    match_html += '</div></a>';
                    match_html += '<div class="mix-pro-price" ><div><h4>'+value.product_type+'</h4><p>Sleek designed card holders which slide into any sized pocket and bag.</p></div>';
                    match_html += '<h5>'+window.Shopify.formatMoney(value2.price, $('body').data('money-format')).replace('.00','').replace(',00','')+'</h5>';
                    match_html += '<form method="post" action="/cart/add">';
                    match_html += '<input type="hidden" name="properties[Letters]" value="'+curr_pro_intial+'">';
                    match_html += '<input type="hidden" name="properties[Letter Color]" value="'+curr_pro_color+'">';
            ...
            jQuery('.mix-match-products > ul').html(match_html);
